...
const char	*see_name(int pla, int plb);
int	fpbn(char *name);
...
const char	*make_title(const char *title, const char *name);
char	*make_rank(int player);
char	*std_title(int level, Boolean sex);
Boolean	reset_mobile(int mobile);
void	p_crapup(int player, const char *str, int flags);
void	crapup(const char *str,int flags);
void	xcrapup(const char *str,Boolean saveflag);
void	death_msg(int plr);
void	loseme(Boolean saveflag);
const char	*lev2s(char *b,int lvl);
int	tscale(void);
...
const char	*str_color (int plr);
const char	*mag_color (int plr);
